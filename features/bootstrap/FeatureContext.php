...
class FeatureContext implements Context
{
	...
	/**
	 * @when Create :nth WordPress site to fix docker issue
	 */
	public function createNNumberOfSite($nth)
	{
		for ( $i=1; $i <= $nth; $i++ ) {
			...
			$this->createWPSite($domain); // Create html site.
		}
	 ...
	 * @return void
	 */
	public function createWPSite($domain)
	{
		$command = sprintf(
			'bin/ee site create %s --type=wp --skip-status-check',
			$domain
		);
